...
from first_interfaces.msg import STMControl, STMState
from first_interfaces.srv import STMSetControlType
...
class STMControlNode(Node):
    def __init__(self):
        ...
        self.declare_parameter('master_group_id', "group_1") 
        self.declare_parameter('slave_group_id', "group_2") 
        ...
        # variables
        self.master_group_id = self.get_parameter("master_group_id")
        self.slave_group_id = self.get_parameter("slave_group_id")

        # 5. TODO: Define topic names for master and slave based on group IDs obtained from parameters
        self.master_state_topic = f"/{self.master_group_id}/stm_state" # Master stm_state topic (/group_id/stm_state)
        self.slave_state_topic = f"/{self.slave_group_id}/stm_state"
        
        self.slave_control_topic = f"/{self.slave_group_id}/stm_control" # Slave stm_control topic (/group_id/stm_control)

        # 6. TODO: Create subscribers for STM state (master and slave)
        self.master_stm_state_subscriber = self.create_subscription(STMState, self.master_state_topic)  # Subscriber for the master node state
        self.get_logger().info(f"Subscribed to {self.master_state_topic}")
        
        self.slave_stm_state_subscriber = self.create_subscription(STMState, self.slave_state_topic)
        self.get_logger().info(f"Subscribed to {self.slave_state_topic}")


        # 7. TODO: Create publishers for STM control (slave)
        self.slave_stm_control_publisher = self.create_publisher(STMControl, self.slave_control_topic)  # Publisher for the slave node control
        ...
        self.control_type_service = self.create_service(STMSetControlType, self.control_type_service_name)

        # 10. TODO: Create a timer for the publisher and to control the STM. 
        self.timer_control_loop = self.create_timer(1/100, self.timer_callback)
